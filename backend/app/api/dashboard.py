...
async def get_dashboard_stats(
    ...
    cache_key = f"dashboard:stats:v3:{vendor}:{store}:{search}:{tags}:{status}:{date_from}:{date_to}"
    try:
        # 1. Try Cache
        ...
        stats_data = await anyio.to_thread.run_sync(lambda: service.get_aggregated_stats(vendor=vendor, store=store, search=search, tags=tags, status=status, date_from=date_from, date_to=date_to))
        designers_data = await anyio.to_thread.run_sync(lambda: service.get_designers_with_counts(store=store, tags=tags, status=status))
